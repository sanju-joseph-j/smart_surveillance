...
# Instead of real MySQL connection
class DummyDB:
    def cursor(self):
        return self
    def execute(self, query, params=None):
        print(f"Skipping DB query: {query}")
    def fetchall(self):
        return []  # return empty list for selects
    def commit(self):
        pass
    def close(self):
        pass

# Replace your MySQL connection with this dummy object
mydb = DummyDB()
...
mail_settings = {
    ...
    "MAIL_USE_TLS": False,
    "MAIL_USE_SSL": True,
    "MAIL_USERNAME": "bgeduscanner@gmail.com",
    "MAIL_PASSWORD": "gitgogxzpcbuhlub"
}
...
def CNN():
    ...
    y, y_test = y.to_categorical(y, 10), y.to_categorical(y_test, 10)
            ...
            if cc1<=cutoff:
                ...
                if v1<3:
                    ...
                    with app.app_context():
                        msg = Message(subject=message, sender=app.config.get("MAIL_USERNAME"),recipients=[email], body=message2)
                        with app.open_resource("static/detect/"+vface) as fp:  
                            msg.attach("static/detect/"+vface, "images/jpeg", fp.read())
                        mail.send(msg)
